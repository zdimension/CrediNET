...
namespace CrediNET
{
    ...
    public class Account
    {
        ...
        private Dictionary<string, System.Windows.Media.Color> _budgets = new Dictionary<string, System.Windows.Media.Color> { { "alimentaire", Colors.White }, { "divers", Colors.White }, { "habitat", Colors.White }, { "santé", Colors.White }, { "salaire", Colors.White }, { "voiture", Colors.White } };

        /// <summary>
        /// The categories of operations
        /// </summary>
        public Dictionary<string, System.Windows.Media.Color> Budgets
        {
        ...
        public static Account FromFile(string fp)
        {
            ...
            else if (Path.GetExtension(fp) == ".cne")
            {
                ...
                {
                    case "de-DE":
                        ins = "Bitte geben Sie das Passwort des Kontos.";
                        break;

                    case "fr-FR":
                        ins = "Veuillez saisir le mot de passe associé au compte.";
                        break;

                    default: //case "en-US":
                        ins = "Please type the account's password below.";
                        break;
                }

                var aeee = FromXmlCode(CryptorEngine.Decrypt(File.ReadAllText(fp), InputBox.Show(ins, ""), true), fp);
                aeee.Encrypted = true;
                return aeee;*/
            }
        ...
        {          
            var doc = new XDocument(new XElement("Compte",
                    ...
                    new XElement("Budgets",
                        cm.Budgets.Select(y => new XElement("B", new XAttribute("color", ColorTranslator.ToHtml(y.Value.ToGDIColor())), y.Key)))));
        ...
        public void SaveSQLite(string filePath)
        {
            ...
            {
                new SQLiteCommand("insert into Budgets (name, color) values ('" + b.Key.Sanitize() + "', '" + ColorTranslator.ToHtml(b.Value.ToGDIColor()) + "')", dbc).ExecuteNonQuery();
            }
            ...
            try
            {
                ...
                while (!good && try_ != 3)
                {
                    ...
                    else
                    {
                        ...
                        var sb = MD5.CreateMD5Hash(InputBox.Show(ins, ""));
